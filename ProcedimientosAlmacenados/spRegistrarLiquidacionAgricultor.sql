...
AS
	BEGIN TRANSACTION
		...
		IF (SELECT CosechaID FROM PagoAgricultores WHERE CosechaID = @cosechaID AND Tipo = 'L') IS NULL
			BEGIN
				DECLARE @pagoID INT;
				...
				DECLARE @tAgricultorInsumos InsumosDeducidos;

				INSERT INTO @tAgricultorInsumos (AgricultorInsumoID) 
				SELECT AgricultorInsumoID FROM dbo.ftInsumosPendientesAgricultor(@agricultorID, @saldo);
				...
				SELECT @deduccionInsumos = (SELECT dbo.fDeduccionInsumosAgricultor(@tAgricultorInsumos));
