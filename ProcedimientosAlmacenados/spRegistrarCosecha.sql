...
AS
	BEGIN TRANSACTION
		...
		VALUES (@cosechaID, @agricultorID, @bodegaID, FORMAT(@fecha, 'dd/MM/yyy'), 0.00);

		IF @@ERROR <> 0 AND @err = 0 SELECT @err = 1;
		
		INSERT INTO CosechaDetalleAgricultor 
		SELECT @cosechaID AS CosechaID, ProductoID, Cantidad, Precio, ISNULL(Descuento, 0) FROM @tdetalle

		IF @@ERROR <> 0 AND @err = 0 SELECT @err = 1;

		DECLARE @total FLOAT;

		SELECT @total = SUM(Precio * Cantidad * (1 - ISNULL(Descuento, 0))) FROM @tdetalle;
